...
const GymContent = ({ initialGyms }) => {
	...
	const { data: gyms = [], isLoading } = useQuery({
		...
		queryFn: async () => {
			const res = await fetch(`/api/gyms?search=${search}`);
	...
	return (
		<>
			<div className="relative z-0 flex h-screen min-h-[800px] sm:min-h-auto max-h-screen w-full items-center justify-center overflow-hidden bg-[url(/gyms/gyms_1024.jpg)] bg-no-repeat xl:block lg:bg-none">
				<Image
				...
				/>
				<Image
					className="hidden xl:block lg:h-auto lg:object-contain absolute left-0 bottom-[-40px]  rounded-tr-[40%] z-0"
					src="/gyms/gyms_2.jpg"
					alt="climber on boulder"
					width={640}
					height={480}
				/>
				<div className="flex xl:w-[900px] max-w-[90%] flex-col gap-8 rounded-2xl xl:gap-12 z-20  lg:left-[9%] absolute bottom-[440px]">
