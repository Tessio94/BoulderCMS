...
const GalleryContent = ({ initialGalleries }) => {
	...
	const { data: galleries = [], isLoading } = useQuery({
		...
		queryFn: async () => {
			const res = await fetch(`/api/galleries?search=${search}`);
	...
	return (
		<>
			<div className="relative z-0 flex h-screen min-h-[800px] max-h-[calc(100vh-125px)] w-full items-center justify-center overflow-hidden bg-[url(/homepage/gallery_1024.jpg)] bg-no-repeat lg:block lg:h-fit lg:min-h-[unset] lg:bg-none">
				...
				<div className="absolute bottom-[21%] left-[50%] flex w-[800px] max-w-[90%] translate-x-[-50%] flex-col gap-8 rounded-2xl xl:gap-12 z-20">
					<div className="flex flex-col items-center gap-14 sm:gap-10 xl:gap-12">
						...
						<div className="flex w-full items-center shadow-2xl shadow-cyan-900 rounded-2xl">
