...
const LoginForm = ({ user }: UserProfileProps) => {
  ...
  return (
    <div className="relative flex h-screen min-h-[1080px] w-full items-center justify-center bg-[url(/homepage/boulder_1920.jpg)] bg-cover bg-no-repeat shadow-2xl shadow-amber-400/40">
      {user ? (
        <div className="relative w-[600px] max-w-[90%] rounded-2xl border-t-4 border-t-cyan-900 bg-gradient-to-b from-cyan-900/10 via-cyan-200/60 to-amber-400/50 px-2 py-7 shadow-2xl shadow-amber-400/40 backdrop-blur-md">
          <p className="my-text-stroke relative mx-auto mb-16 w-fit text-center text-3xl font-extrabold text-cyan-900 after:absolute after:top-[calc(100%+10px)] after:left-[50%] after:h-[5px] after:w-[20%] after:translate-x-[-50%] after:rounded-2xl after:border-[1px] after:border-cyan-900 after:bg-amber-400 after:content-['']">
            You are already logged in
          </p>
          <p className="mb-10 text-center text-xl text-cyan-900">
            Logout and login with different profile
          </p>
          <button
            onClick={handleLogout}
            className="mx-auto block cursor-pointer rounded-2xl border-[2px] border-transparent bg-cyan-900 px-5 py-2 text-cyan-200 transition-all duration-500 hover:border-cyan-900 hover:bg-cyan-200 hover:text-cyan-900"
          >
            Logout
          </button>
        </div>
